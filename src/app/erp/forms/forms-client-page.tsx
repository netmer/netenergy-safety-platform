...
import type { RegistrationForm, RegistrationFormField, RegistrationFormSubField, RegistrationFormFieldOption } from '@/lib/course-data';
...
import { 
    MoreHorizontal, PlusCircle, Pencil, Trash2, Loader2, FileText, Type, Heading1, 
    RectangleHorizontal, Users, Mail, Phone, Home, Copy, GripVertical, Plus, 
    Building, User, AlignLeft, List, CheckCircle2, ChevronDown, Radio, AlertCircle
} from 'lucide-react';
...
import { createForm, updateForm, deleteForm, duplicateForm, type FormState } from './actions';
...
function EditableField({ index, field, isSelected, onSelect, onUpdate, onRemove, onDuplicate }) {
    ...
    const addSubField = (type: RegistrationFormSubField['type'] = 'text') => {
        ...
        handleFieldChange('subFields', newSubFields);
    };
    ...
    const handleOptionChange = (optionIndex: number, prop: keyof RegistrationFormFieldOption, value: string) => {
    ...
    const addOption = () => {
        const newOptions = [...(field.options || []), { id: nanoid(), label: `ตัวเลือกที่ ${(field.options?.length || 0) + 1}`, value: `option_${(field.options?.length || 0) + 1}` }];
        handleFieldChange('options', newOptions);
    };
    ...
    const renderPreview = () => {
        switch (type) {
            ...
            case 'coordinator':
                 return (
                     <div>
                        ...
                        <div className="mt-2 space-y-4 p-4 border rounded-md bg-white dark:bg-black/20 text-sm text-muted-foreground text-left">
                     ...
                     <div className="text-left">
                        ...
                        <div className="mt-2 space-y-4 p-4 border rounded-md bg-white dark:bg-black/20 text-sm text-muted-foreground">
                            ...
                            <div className="grid grid-cols-2 gap-2">
                                ...
                                <Input placeholder="เลขประจำตัวผู้เสียภาษี / เลขบัตรประชาชน" disabled />
                            </div>
            ...
            case 'select':
                return (
                    <div className="text-left">
                        <Label className="font-bold">{field.label || 'เลือกรายการ'}{field.required && <span className="text-destructive"> *</span>}</Label>
                        <Select disabled>
            ...
            case 'radio':
                return (
                    <div className="text-left">
                        <Label className="font-bold mb-3 block">{field.label || 'ตัวเลือกตอบ'}{field.required && <span className="text-destructive"> *</span>}</Label>
                        <RadioGroup disabled className="space-y-2">
                            {(field.options || [{id: '1', label: 'ตัวอย่างตัวเลือก'}]).map((opt) => (
                                <div key={opt.id} className="flex items-center space-x-2">
                                    <RadioGroupItem value={opt.value} id={`prev-${opt.id}`} />
                                    <Label htmlFor={`prev-${opt.id}`}>{opt.label}</Label>
                                </div>
                            ))}
                        </RadioGroup>
                    </div>
            ...
            case 'textarea':
                return (
                    <div className="text-left">
                        <Label className="font-bold">{field.label || 'ข้อความยาว'}{field.required && <span className="text-destructive"> *</span>}</Label>
                        <Textarea className="mt-2" placeholder={field.placeholder || 'กรอกข้อมูล...'} disabled />
                    </div>
                );
            default:
                return (
                ...
                <>
                    <div className="flex items-start gap-4">
                         <div className="flex-1 space-y-2">
                            <Label className="text-xs font-bold text-muted-foreground uppercase tracking-wider">ป้ายกำกับคำถาม</Label>
                            <Input value={field.label} onChange={(e) => handleFieldChange('label', e.target.value)} className="text-base font-bold" placeholder="ป้ายกำกับฟิลด์" />
                         </div>
                    </div>
                    
                    {['text', 'textarea', 'email', 'tel'].includes(type) && (
                        <div className="space-y-2">
                            <Label className="text-xs font-bold text-muted-foreground uppercase tracking-wider">คำอธิบายประกอบ (Placeholder)</Label>
                            <Input value={field.placeholder} onChange={(e) => handleFieldChange('placeholder', e.target.value)} placeholder="ระบุข้อความคำใบ้ในช่อง..." />
                        </div>
                    )}

                    {['select', 'radio'].includes(type) && (
                        <div className="space-y-4 pt-2">
                            <Label className="font-bold">จัดการตัวเลือก (Options)</Label>
                            <div className="space-y-2">
                                {(field.options || []).map((option, optIdx) => (
                                    <div key={option.id} className="flex items-center gap-2">
                                        <div className="flex-1">
                                            <Input value={option.label} onChange={(e) => handleOptionChange(optIdx, 'label', e.target.value)} placeholder="ชื่อตัวเลือก" className="h-9"/>
                                        </div>
                                        <div className="flex-1">
                                            <Input value={option.value} onChange={(e) => handleOptionChange(optIdx, 'value', e.target.value)} placeholder="Value (หลังบ้าน)" className="h-9 font-mono text-xs"/>
                                        </div>
                                        <Button type="button" variant="ghost" size="icon" className="h-8 w-8 text-destructive" onClick={() => removeOption(optIdx)}>
                                            <Trash2 className="h-4 w-4"/>
                                        </Button>
                                    </div>
                                ))}
                            </div>
                            <Button type="button" variant="outline" size="sm" className="w-full border-dashed" onClick={addOption}>
...
function FormBuilder({ defaultValues, formAction }) {
    ...
    const addField = (type: RegistrationFormField['type']) => {
        ...
        switch (type) {
            ...
            case 'page_break':
                ...
                break;
            case 'select':
                newField.label = 'คำถามแบบ Dropdown';
                newField.options = [
                    { id: nanoid(), label: 'ตัวเลือก 1', value: 'option_1' },
                    { id: nanoid(), label: 'ตัวเลือก 2', value: 'option_2' }
                ];
                break;
            case 'radio':
                newField.label = 'คำถามแบบเลือกตอบ (Radio)';
                newField.options = [
                    { id: nanoid(), label: 'ตัวเลือก 1', value: 'option_1' },
                    { id: nanoid(), label: 'ตัวเลือก 2', value: 'option_2' }
                ];
                break;
            case 'text':
                newField.label = 'คำถามแบบพิมพ์ตอบสั้น';
                break;
            case 'textarea':
                newField.label = 'คำถามแบบพิมพ์ตอบยาว';
                break;
        }
    ...
    const questionActions = [
        { type: 'text', label: 'คำถามข้อความสั้น', icon: Type },
        { type: 'textarea', label: 'คำถามข้อความยาว', icon: AlignLeft },
        { type: 'select', label: 'Dropdown List', icon: ChevronDown },
        { type: 'radio', label: 'Radio (เลือกข้อเดียว)', icon: Radio },
    ] as const;
        ...
        <form action={formAction}>
             ...
             <div className="grid grid-cols-12 gap-8">
                 ...
                 <div className="col-span-12 md:col-span-4 lg:col-span-3">
                    <Card className="sticky top-4 p-2 text-left">
                        <div className="space-y-6">
                            <section>
                                <CardHeader className='p-2 pt-0'>
                                    <CardTitle className="text-xs uppercase tracking-widest font-black text-slate-400">บล็อกข้อมูลมาตรฐาน</CardTitle>
                                </CardHeader>
                                <div className="flex flex-col gap-1">
                                    {toolbarActions.map(({ type, label, icon: Icon }) => (
                                        <Button key={type} type="button" variant="ghost" className="justify-start gap-2 h-9 text-xs" onClick={() => addField(type)}>
                            ...
                            <section>
                                <CardHeader className='p-2 pt-0'>
                                    <CardTitle className="text-xs uppercase tracking-widest font-black text-slate-400">ฟิลด์คำถามทั่วไป</CardTitle>
                                </CardHeader>
                                <div className="flex flex-col gap-1">
                                    {questionActions.map(({ type, label, icon: Icon }) => (
                                        <Button key={type} type="button" variant="ghost" className="justify-start gap-2 h-9 text-xs" onClick={() => addField(type as any)}>
                                            <Icon className="h-4 w-4" />
                                            <span>{label}</span>
                                        </Button>
                                    ))}
                            ...
                            <section>
                                <CardHeader className='p-2 pt-0'>
                                    <CardTitle className="text-xs uppercase tracking-widest font-black text-slate-400">เครื่องมือจัดหน้า</CardTitle>
                                </CardHeader>
                                <div className="flex flex-col gap-1">
    ...
    return (
        <>
                                    ...
                                    <TableCell>
                                        <DropdownMenu>
                                            ...
                                            <DropdownMenuContent align="end">
                                                ...
                                                <DropdownMenuItem onClick={() => handleDuplicate(form.id)} disabled={isPending}>
            ...
            <Dialog open={isFormOpen} onOpenChange={(isOpen) => { if (!isOpen) handleCloseDialogs(); else setIsFormOpen(true);}}>
                <DialogContent className="max-w-6xl h-[90vh] flex flex-col p-0 rounded-[2rem] overflow-hidden border-none shadow-2xl">
                    <DialogHeader className="p-8 pb-4 bg-white dark:bg-slate-950 border-b">
                        <DialogTitle className="flex items-center gap-2 text-2xl">
                            <FileText className="text-primary" />
                            {formToEdit ? 'แก้ไขแบบฟอร์ม' : 'สร้างแบบฟอร์มใหม่'}
                        </DialogTitle>
                    </DialogHeader>
                    <div className="flex-1 overflow-y-auto px-8 py-6 bg-slate-50 dark:bg-slate-900/50 custom-scrollbar">
                        <FormBuilder
            ...
            <AlertDialog open={isDeleteAlertOpen} onOpenChange={(isOpen) => { if (!isOpen) handleCloseDialogs(); else setIsDeleteAlertOpen(true);}}>
                <AlertDialogContent className="rounded-[2.5rem] shadow-2xl border-none">
                    ...
                    <AlertDialogFooter className="gap-3 mt-6">
                        ...
                        <AlertDialogAction onClick={handleDeleteConfirm} disabled={isPending} className="bg-destructive hover:bg-destructive/90 rounded-xl h-12 font-bold px-8">
